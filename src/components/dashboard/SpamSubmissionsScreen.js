...
import { spamSubmissionsAPI } from '../../utils/api';
import ErrorModal from '../common/ErrorModal';
import { exportSpamSubmissionsToExcel } from '../../utils/excelExport';
...
const SpamSubmissionsScreen = () => {
  ...
  // Load spam form responses from API
  const loadSpamFormResponses = async () => {
    setLoading(true);
    setError('');
    try {
      const response = await spamSubmissionsAPI.getSpamFormResponses();
      ...
      console.error('Error loading spam form responses:', error);
      setError(error.message || 'Failed to load spam form responses');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Export filtered spam submissions to Excel
  const exportToExcelHandler = () => {
    try {
      exportSpamSubmissionsToExcel(filteredSubmissions, 'spam_submissions_export');
    } catch (error) {
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row mb-6 gap-4 items-center bg-white/60 p-4 rounded-lg shadow">
        ...
        <button
          ...
          className="w-full md:w-auto px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium"
        >
              ...
              {filteredSubmissions.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center text-gray-500 py-8">
                    {submissions.length === 0 ? 'No spam submissions found.' : 'No spam submissions match the current filters.'}
                  </td>
                </tr>
              ) : (
                filteredSubmissions.map((sub) => (
                  <tr key={sub.uniqueKey || sub.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4">
                      <button
                        className="flex items-center gap-1 bg-red-100 hover:bg-red-300 text-red-800 px-4 py-1.5 rounded text-xs font-medium shadow transition-colors duration-150"
...
export default SpamSubmissionsScreen;
